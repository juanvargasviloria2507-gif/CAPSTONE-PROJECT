/* views/home.js
   Vista principal: hero + grilla de productos, filtrable por categoría
   mediante el parámetro de ruta ":category" (usado por el nav).
*/
...
function renderHome(container, params) {
  ...
  var heading = categoryLabel ? categoryLabel.label : 'Todos los regalos';

  container.innerHTML =
    '<section class="hero">' +
      '<div class="hero-copy">' +
        '<h1>Encuentra el regalo perfecto para cada ocasión</h1>' +
        '<p>Descubre regalos únicos y pensados con cariño que harán sonreír a tus seres queridos. Desde tesoros personalizados hasta clásicos atemporales, tenemos algo especial para todos.</p>' +
        '<div class="hero-actions">' +
          '<button class="btn btn-primary" id="shop-now-btn">Comprar ahora</button>' +
          '<button class="btn btn-secondary">Guía de regalos</button>' +
        '</div>' +
      '</div>' +
...
function renderProductCard(p) {
  return (
    '<div class="product-card">' +
      ...
      '</div>' +
      '<button class="add-to-cart" data-add-to-cart="' + p.id + '">' + ICONS.cart + ' Añadir al carrito</button>' +
    '</div>'
  );
}
